...
class Augmentaion:
    ...
    def train_augmentationV1(
        self, file_name="train_augmentV1.csv", save=True
    ...
    def train_augmentationV2(
        self, file_name="train_augmentV2.csv", save=True
    ) -> Union[pd.DataFrame, None]:
        ...
        df_sr = self.synonym_replacement(
            df=df_spellcheck,
            wordnet_path=self.wordnet_path,
            symmin=3.0,
            symmax=4.5,
            rng=0.5,
            ratio=2,
    ...
    def train_augmentationV3(
        self, file_name="train_augmentV3.csv", save=True
    ) -> Union[pd.DataFrame, None]:
    ...
    def spelling_check(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_change = df.copy()
        tqdm.pandas()
        df_change["sentence_1"] = df_change["sentence_1"].progress_map(
            self.apply_hanspell
        )
        df_change["sentence_2"] = df_change["sentence_2"].progress_map(
            self.apply_hanspell
        )
        return df_change
    ...
    def synonym_replacement(
        ...
        for i in tqdm(range(len(n1)), desc="Synonym Replacement"):
            ...
            for same_noun in same_nouns_set:
                ...
                if (
                    len(same_nouns_set) > 1
                    and same_noun in wordnet
                    and len(wordnet[same_noun]) >= 2
                ):
    ...
    def swap_sentence(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_swapped = df_swapped[
            ((df_swapped["label"] >= 0.5) & (df_swapped["label"] < 3.5))
    ...
    def copied_sentenceV2(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        sample_0005 = df_change[
            (df_change["label"] >= 0.0) & (df_change["label"] < 0.5)
        ][-1779:]
        sample_3035 = df_change[
            (df_change["label"] >= 3.0) & (df_change["label"] < 3.5)
        ][-508:]
        sample_0445 = df_change[
            (df_change["label"] >= 1.0) & (df_change["label"] < 1.5)
        ][-414:]
...
if __name__ == "__main__":
    ...
    """
    train_augment = Augmentaion(
        data_path=Path.TRAIN.value,
        save_path=Path.SAVE.value,
        wordnet_path=Path.WORDNET.value,
    )

    dev_augment = Augmentaion(
        data_path=Path.DEV.value,
        save_path=Path.SAVE.value,
        wordnet_path=Path.WORDNET.value,
    )

    test_augment = Augmentaion(
        data_path=Path.TEST.value,
        save_path=Path.SAVE.value,
        wordnet_path=Path.WORDNET.value,
    )
