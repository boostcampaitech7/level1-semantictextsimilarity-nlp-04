...
class Augmentaion():
    ...
    def train_augmentationV4(self, file_name='train_augmentV4.csv', save=True) -> Union[pd.DataFrame, None]:
        """AugmentationV4 버전 데이터 생성
    - 라벨 0.5-2.5 데이터 증강.
    - 원본 데이터 + 맞춤법 검사 데이터 + 동의어 교체 데이터 + 문장 스왑 데이터

    Args:
        file_name (str, optional): 저장할 파일 이름. Defaults to 'train_augmentV4.csv'.
        save (bool, optional): 반환 or 저장 여부 선택. Defaults to True.
    ...
    Returns:
        ...
        # 라벨이 0.5 이상 2.5 이하인 데이터 필터링
        df_filtered = df_original[(df_original['label'] >= 0.5) & (df_original['label'] <= 2.5)].reset_index(drop=True)
    
        # 맞춤법 검사 데이터 생성
        df_spellcheck = self.spelling_check(df_filtered)
    
        # 동의어 교체 데이터 생성
        df_synonym = self.synonym_replacement(df_filtered, wordnet_path=self.wordnet_path, symmin=0.5, symmax=2.5, ratio=2)
    
        # 문장 스왑 데이터 생성
        df_swap = self.swap_sentence(df_filtered)
    
        # 모든 증강 데이터 결합
        df_augment = self.concat([df_filtered, df_spellcheck, df_synonym, df_swap,df_original])
    ...
    def train_augmentationV5(self, file_name='train_augmentV5.csv', save=True) -> Union[pd.DataFrame, None]:
        """AugmentationV5 버전 데이터 생성
    - 라벨 2.5-5 데이터 증강.
    - 원본 데이터 + 맞춤법 검사 데이터 + 동의어 교체 데이터 + 문장 스왑 데이터 + 0-0.5 구간 데이터로 5점 데이터 생성

    Args:
        file_name (str, optional): 저장할 파일 이름. Defaults to 'train_augmentV5.csv'.
        save (bool, optional): 반환 or 저장 여부 선택. Defaults to True.
    ...
    Returns:
        ...
        """
        df_original = self.df.copy()
        df_change = df_original.reset_index(drop=True).copy()
        sample_0005 = df_change[(df_change['label'] >= 0.0) &(df_change['label'] < 0.5)][-1750:]

        for index in [sample_0005.index] :
        ...
        # # 라벨이 2.5 이상인 데이터 필터링
        df_filtered = df_original[(df_original['label'] > 2.5)].reset_index(drop=True)
    
        # # 맞춤법 검사 데이터 생성
        df_spellcheck = self.spelling_check(df_filtered)
    
         # 동의어 교체 데이터 생성
        df_synonym = self.synonym_replacement(df_filtered, wordnet_path=self.wordnet_path, symmin=2.5, symmax=5.0, ratio=2)
    
        # # 문장 스왑 데이터 생성
        df_swap = self.swap_sentence(df_filtered)
    
        # # 최종 데이터 생성
        df_augment = self.concat([df_change, df_original,df_swap,df_spellcheck,df_synonym])  # 원본 데이터 포함
    ...
    def train_augmentationV7(self, file_name='train_augmentV7.csv', save=True) -> Union[pd.DataFrame, None]:
        """AugmentationV7 버전 데이터 생성
        - V3 데이터-> dev랑 비율 같게 맞춰주기 

    Args:
        file_name (str, optional): 저장할 파일 이름. Defaults to 'train_augmentV7.csv'.
        save (bool, optional): 반환 or 저장 여부 선택. Defaults to True.
    ...
    Returns:
        ...
        if 'train_augmentV3.csv' in os.listdir('/data/ephemeral/home/gayeon2/data'):
            print('train_augmentV3.csv is already exist!! load....')
            df_augmentV3 = pd.read_csv('/data/ephemeral/home/gayeon2/data/train_augmentV3.csv')
        else:
            df_augmentV3 = self.train_augmentationV3(save=False)
        

        # 전체 데이터 개수 계산
        total_samples = len(df_augmentV3)
        ...
        for label_bin, count in label_ratios.items():
            # 라벨 구간을 추출
            lower_bound, upper_bound = map(float, label_bin.split('-'))
            filtered_df = df_augmentV3[(df_augmentV3['label'] >= lower_bound) & (df_augmentV3['label'] < upper_bound)]
        
            # 비율에 따라 필요한 개수 계산
            required_count = int((count / total_ratio) * total_samples)
        
            # 필요한 개수만큼 샘플링
            sampled_df = filtered_df.sample(n=min(required_count, len(filtered_df)), random_state=42)
        
            df_final = pd.concat([df_final, sampled_df], ignore_index=True)

        if save:
            self.save_data(df_final, self.save_path, file_name)
        else:
            return df_final
    ...
    def under_sampling(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df.copy()
    ...
    def swap_sentence(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_swapped = df_swapped[((df_swapped['label'] >= 0.5) & (df_swapped['label'] < 3.5)) | 
                             ((df_swapped['label'] >= 4.5) & (df_swapped['label'] < 5))]
        return df_swapped

    def copy_sentence(self, df: pd.DataFrame, index_min=250, index_max=750) -> pd.DataFrame:
        df=df.copy()
    ...
    def train_augmentationV6(self, file_name='train_augmentV6.csv', save=True) -> Union[pd.DataFrame, None]:
        """AugmentationV6 버전 데이터 생성
    - 언더샘플링 및 문장 스왑, 문장 복사 데이터 증강

    Args:
        file_name (str, optional): 저장할 파일 이름. Defaults to 'train_augmentV6.csv'.
        save (bool, optional): 반환 or 저장 여부 선택. Defaults to True.
    ...
    Returns:
        ...
        # 언더샘플링 데이터 생성
        df_under_sampled = self.under_sampling(df_original)  
        df_swapped_sentence = self.swap_sentence(df_original)  
        df_copied_sentence = self.copy_sentence(df_original)  

        # 최종 데이터 생성
        df_augment = self.concat([df_under_sampled, df_swapped_sentence, df_copied_sentence])  # 리스트로 전달
...
class Path(Enum):
    TRAIN = '/data/ephemeral/home/gayeon2/data/train.csv'
    DEV = '/data/ephemeral/home/gayeon2/data/dev.csv'
    TEST = '/data/ephemeral/home/gayeon2/data/test.csv'
    WORDNET = '/data/ephemeral/home/gayeon2/data/wordnet.pickle'
    SAVE = '/data/ephemeral/home/gayeon2/data'
...
if __name__ == '__main__':
    ...
    #train_augment.train_augmentationV4(file_name='train_augmentV4.csv', save=True)
    #train_augment.train_augmentationV5(file_name='train_augmentV5.csv', save=True)
    #train_augment.train_augmentationV6(file_name='train_augmentV6.csv',save=True)
    train_augment.train_augmentationV7(file_name='train_augmentV7.csv',save=True)
